...
func main() {
	...
	if len(flag.Args()) == 0 || opts.subtreePID != "" {
		...
		// at least one PID in each user namespace. This enables
		// us to discover the UID and GID map for each user namespace,
		// so do that discovery in order that we can show display
		// the maps. We don't do this if only some of the PIDs on
		// the system are scanned, since then it's likely that we
		// gathered information about some user namespaces without
		// discovering any of their member processes.
